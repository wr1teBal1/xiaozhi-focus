...
//################
    xTaskCreate([](void* arg) {
        ...
        app->SerialDetect();
        vTaskDelete(NULL);
    }, "serial_detect", 4096 * 2, this, 3, nullptr);
...
void Application::SerialDetect(){
    Serial *serial = Serial::GetInstance();
    while(1)
    {
       
        serial->SerialReceive([this]()
                              { 
            uint8_t rx_buf[10] = {0};
            int len = uart_read_bytes(UART1_PORT_NUM, rx_buf, sizeof(uint8_t), pdMS_TO_TICKS(100)); 
            if(len>0)
